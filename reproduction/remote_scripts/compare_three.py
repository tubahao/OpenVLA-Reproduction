...
for name, ckpt in [("BASE", BASE), ("OURS", OURS), ("OFFICIAL", OFFICIAL)]:
    ...
    key = "libero_spatial" if "libero_spatial" in keys else ("libero_spatial_no_noops" if "libero_spatial_no_noops" in keys else "bridge_orig")
    inputs = proc(prompt, img).to("cuda:0", dtype=torch.bfloat16)
    with torch.inference_mode():
        act = model.predict_action(**inputs, unnorm_key=key, do_sample=False)
    print(name, "keys=", keys[:4], "->", np.round(act, 4).tolist(), flush=True)
    del model, proc
