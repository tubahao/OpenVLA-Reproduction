...
print("STEP_RANGE", steps[0], "->", steps[-1], "count_expected", (steps[-1] - steps[0]) // 10 + 1)
prev = None
gaps = []
for s in steps:
    if prev is not None and s - prev != 10:
        gaps.append((prev, s))
    prev = s
print("GAPS", gaps[:10], "N_GAPS", len(gaps))
for r in rows[::200]:
    print("step", r["_step"], "| loss", round(r.get("train_loss", float("nan")), 3),
          "| acc", round(r.get("action_accuracy", float("nan")), 3),
          "| l1", round(r.get("l1_loss", float("nan")), 3))
if rows:
    r = rows[-1]
    print("FINAL step", r["_step"], "| loss", round(r.get("train_loss", float("nan")), 3),
          "| acc", round(r.get("action_accuracy", float("nan")), 3),
          "| l1", round(r.get("l1_loss", float("nan")), 3))
